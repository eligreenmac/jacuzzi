...
function LoginForm() {
  ...
  const [error, setError] = useState(
    searchParams.get("error") === "google_not_configured"
      ...
      ? "התחברות עם Google נכשלה, נסה שנית או התחבר עם אימייל."
      : ""
  );
  const [loading, setLoading] = useState(false);
  const [isGoogleModalOpen, setIsGoogleModalOpen] = useState(searchParams.get("google_prompt") === "1");
  const [googleEmailInput, setGoogleEmailInput] = useState("eligreenmail@gmail.com");
  ...
  const handleGoogleFastLogin = async (targetEmail?: string) => {
    const emailToUse = (targetEmail || googleEmailInput || email).trim().toLowerCase();
    if (!emailToUse) {
      setError("נא להזין כתובת Google חוקית");
      return;
    }
    ...
    try {
      const res = await fetch("/api/auth/login", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ email: emailToUse, isGoogleLogin: true }),
      });

      const data = await res.json();

      if (!res.ok) {
        throw new Error(data.error || "התחברות עם Google נכשלה");
      }
  ...
  return (
    <div className="bg-slate-900/90 border border-slate-800 rounded-3xl p-8 shadow-2xl space-y-6">
      ...
      <button
        type="button"
        onClick={() => {
          // If Google Client ID not setup yet on cloud, open direct 1-click modal
          handleGoogleLogin();
        }}
      ...
      {/* 1-Click Fast Google Login Dialog */}
      {isGoogleModalOpen && (
        <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4 animate-in fade-in duration-200">
          <div className="bg-slate-900 border border-cyan-800/80 rounded-3xl p-6 sm:p-8 max-w-md w-full space-y-5 shadow-2xl">
            <div className="text-center space-y-2">
              ...
              <h3 className="text-lg font-bold text-white">כניסה והרשמה מיידית עם Google</h3>
              <p className="text-xs text-slate-400">הזן את כתובת ה-Google שלך לכניסה מהירה ומאובטחת ללא צורך בסיסמה</p>
            </div>
            ...
            <div className="space-y-3">
              <input
                ...
                className="w-full bg-slate-950 border border-slate-700 rounded-xl px-4 py-3 text-white text-sm focus:border-cyan-400 focus:outline-none text-center font-semibold"
              />
              ...
              <button
                ...
                className="w-full py-3 rounded-xl bg-gradient-to-r from-blue-600 to-cyan-600 hover:from-blue-500 hover:to-cyan-500 text-white font-bold text-sm shadow-lg flex items-center justify-center gap-2 transition-all disabled:opacity-50"
              >
                {loading ? "מתחבר..." : "✓ היכנס עכשיו לחשבון"}
              </button>
