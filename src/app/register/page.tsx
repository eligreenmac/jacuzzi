...
export default function RegisterPage() {
  ...
  const handleGoogleFastRegister = async () => {
    const emailToUse = (googleEmailInput || formData.email).trim().toLowerCase();
    if (!emailToUse) {
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error(data.error || "הרשמה עם Google נכשלה");
      }
  ...
  return (
    <div className="max-w-xl mx-auto py-8">
      <div className="bg-slate-900/90 border border-slate-800 rounded-3xl p-8 shadow-2xl space-y-6">
        ...
        {/* 1-Click Fast Google Register Dialog */}
        {isGoogleModalOpen && (
          <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4 animate-in fade-in duration-200">
            <div className="bg-slate-900 border border-cyan-800/80 rounded-3xl p-6 sm:p-8 max-w-md w-full space-y-5 shadow-2xl">
              <div className="text-center space-y-2">
                ...
                <h3 className="text-lg font-bold text-white">הרשמה מיידית עם חשבון Google</h3>
                <p className="text-xs text-slate-400">הזן את כתובת ה-Google שלך להרשמה מיידית והגדרת הג'קוזי</p>
              </div>
              ...
              <div className="space-y-3">
                ...
                <button
                  type="button"
                  disabled={loading}
                  onClick={() => handleGoogleFastRegister()}
                ...
                >
                  {loading ? "יוצר חשבון..." : "✓ צור חשבון Google עכשיו"}
                </button>
